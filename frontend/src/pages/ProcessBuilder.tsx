...
async function searchGraphUsers(query: string): Promise<GraphUser[]> {
  const response = await api.searchDirectoryUsers(query);
             ...
             <div style={fieldSummaryRow}>
              ...
              <span style={itemText}>
               ...
               <small style={smallEllipsis}>
                                {cleanText(field.key, 'campo')}{field.required ? ' - Obligatorio' : ' - Opcional'}
               </small>
              </span>
       ...
       {showStageEditor && currentNode && (
        <StageEditorModal
         ...
         onUpdate={patch => updateNode(selectedNodeIndex, patch)}
                  onSearchUsers={searchGraphUsers}
        />
       )}
       {activeTab === 'publish' && (
...
function StageEditorModal({
  ...
  onSearchUsers: (query: string) => Promise<GraphUser[]>;
}) {
  const [approverQuery, setApproverQuery] = useState('');
  const [approverResults, setApproverResults] = useState<GraphUser[]>([]);
  const [approverLoading, setApproverLoading] = useState(false);
  const [approverError, setApproverError] = useState('');

  async function runApproverSearch() {
    const q = approverQuery.trim() || node.approver_email || '';

    if (q.trim().length < 2) {
      setApproverError('Escribe al menos 2 caracteres.');
      return;
    }
    ...
    try {
      ...
      setApproverResults(results);

      if (results.length === 0) {
        setApproverError('No se encontraron usuarios.');
      }
    } catch (e) {
      setApproverError(e instanceof Error ? e.message : 'No se pudo buscar el aprobador.');
    } finally {
      setApproverLoading(false);
    }
  }

  function selectApprover(user: GraphUser) {
    const email = user.mail || user.userPrincipalName || '';

    if (!email) {
      setApproverError('El usuario seleccionado no tiene correo disponible.');
      return;
    }
    ...
    setApproverQuery(user.displayName || email);
    setApproverResults([]);
    setApproverError('');
  }
  ...
  return (
    <div style={modalBackdrop}>
      <div style={stageModalCard} className="flow-tour-card">
        ...
        <div style={formGrid}>
          <Field label="Responsable">
            <select
              ...
              onChange={e => {
                const value = e.target.value as ProposalNode['approver_type'];

                onUpdate({
                  approver_type: value,
                  approver_email: value === 'email' ? node.approver_email || '' : '',
          ...
          {(node.approver_type || 'email') === 'email' && (
            <Field label="Aprobador seleccionado">
              <input
                value={node.approver_email || ''}
                readOnly
                style={input}
                placeholder="Selecciona un usuario real"
              />
            </Field>
          )}
        </div>
        ...
        {(node.approver_type || 'email') === 'email' && (
          <div style={approverSearchBox}>
            <div style={panelTitle}>Buscar aprobador en Microsoft</div>

            <div style={approverSearchRow}>
              <input
                value={approverQuery}
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    void runApproverSearch();
                  }
                }}
                style={input}
                placeholder="Nombre o correo del aprobador"
              />

              <button
                type="button"
                onClick={() => void runApproverSearch()}
                disabled={approverLoading}
                style={secondarySmallButton}
              >
                {approverLoading ? 'Buscando...' : 'Buscar'}
              </button>
            </div>

            {approverError && (
              <div style={approverErrorBox}>{approverError}</div>
            )}

            {approverResults.length > 0 && (
              <div style={approverResultsBox}>
                {approverResults.map(user => {
                  ...
                  return (
                    <button
                      ...
                      style={approverResultItem}
                    >
                      <span style={itemText}>
                        <strong style={ellipsis}>{cleanText(user.displayName, 'Usuario')}</strong>
                        <small style={smallEllipsis}>{email}</small>
            ...
            {!isValidApproverEmail(node.approver_email) && (
              <div style={warningBox}>
                Esta etapa necesita un aprobador real antes de publicar.
              </div>
            )}
          </div>
        )}
...
function isFakeApproverEmail(value: unknown): boolean {
  const email = String(value ?? '').trim().toLowerCase();

  return [
    'compras@metrored.med.ec',
    'contabilidad@metrored.med.ec',
    'administracion@metrored.med.ec',
    'marketing@metrored.med.ec',
    'mantenimiento@metrored.med.ec',
    'supervisora@metrored.med.ec',
  ].includes(email);
}

function isValidApproverEmail(value: unknown): boolean {
  const email = String(value ?? '').trim().toLowerCase();

  if (!email) return false;
  if (isFakeApproverEmail(email)) return false;
...
function getChecklist(proposal: Proposal | null): Array<{ label: string; ok: boolean; detail: string }> {
 ...
 const allOwners = nodes.length > 0 && nodes.every(node => {
    if (node.approver_type === 'requester') return true;
    if (node.approver_type === 'role') return Boolean(node.role?.trim());
    return isValidApproverEmail(node.approver_email);
  });
...
function cleanText(value: unknown, fallback = ''): string {
  ...
  const hasMojibake = badCodes.some(code => text.includes(String.fromCharCode(code)));
...
function ownerLabel(node: ProposalNode): string {
  ...
  if (!isValidApproverEmail(node.approver_email)) {
    return 'Aprobador pendiente';
  }

  return node.approver_email || 'Aprobador pendiente';
}
...
const approverSearchBox: CSSProperties = {
  display: 'grid',
  gap: 8,
  marginTop: 8,
  padding: 10,
...
const approverSearchRow: CSSProperties = {
  display: 'grid',
  gridTemplateColumns: 'minmax(0, 1fr) auto',
  gap: 8,
  alignItems: 'center',
...
const approverResultsBox: CSSProperties = {
  display: 'grid',
  gap: 6,
};

const approverResultItem: CSSProperties = {
  width: '100%',
  display: 'grid',
  padding: 10,
  borderRadius: 10,
...
const approverErrorBox: CSSProperties = {
  padding: 10,
  borderRadius: 10,
  background: '#FFF2EC',
  border: '1px solid #FDA29B',
