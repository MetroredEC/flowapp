...
import {
  Card, PageHeader, StatusBadge, StepBadge, Spinner, Btn, LevelStepper, Field, Input, Textarea
} from '../components/ui';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {data.attachments.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {data.attachments.map(a => (
                  <a key={a.id}
                    href={`${API}/api/files/${encodeURIComponent((a as unknown as { r2_key: string }).r2_key ?? a.id)}`}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: '#aaa' }}>
                        {(a.size_bytes / 1024).toFixed(1)} KB · {a.content_type}
                      </div>
                    </div>
                    {a.is_selected === 1 && (
          ...
          {data.request_type_name === 'Marketing' && (
            <Card>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <h2 style={{ fontSize: 15, fontWeight: 700 }}>Costo de campaña</h2>
              ...
              {data.campaign_cost ? (
                ...
                <p style={{ fontSize: 13, color: '#aaa' }}>
                  {data.status === 'approved'
                    ? 'Solicitud aprobada. Registra el costo de campaña.'
                    : 'El costo de campaña se registra al completar la aprobación.'}
                </p>
              )}
            </Card>
          )}
        ...
        {/* Right col: flujo de aprobación */}
        <Card style={{ alignSelf: 'start' }}>
          <div style={{ marginBottom: 16 }}>
            <h2 style={{ fontSize: 15, fontWeight: 700, marginBottom: 8 }}>Flujo de aprobación</h2>
            <LevelStepper
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        <InfoRow label="Código de campaña"  value={cost.campaign_code} />
        <InfoRow label="Monto total"        value={`${cost.currency} ${cost.total_amount.toLocaleString('es-EC')}`} />
        <InfoRow label="Fecha de ejecución" value={cost.execution_date} />
        <InfoRow label="Fecha de facturación" value={cost.billing_date} />
      </div>
        ...
        <div>
          ...
          {cost.vendors.map((v, i) => (
            <div key={i} style={{
              ...
              borderBottom: '1px solid #F2F2F0', fontSize: 13,
            }}>
              <span style={{ color: '#333' }}>{v.vendor_name}
                {v.is_selected === 1 && <span style={{ marginLeft: 6, fontSize: 11,
                  background: '#E1F5EE', color: '#085041', padding: '1px 6px', borderRadius: 10,
                  fontWeight: 700 }}>Seleccionado</span>}
              </span>
...
function CampaignCostForm({ requestId, onSaved }: { requestId: string; onSaved: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <Field label="Código de campaña">
          <Input value={form.campaign_code} onChange={e => set('campaign_code', e.target.value)} placeholder="MKT-2026-001" />
        </Field>
        <Field label="Monto total">
          <Input type="number" value={form.total_amount} onChange={e => set('total_amount', e.target.value)} placeholder="0.00" />
        </Field>
      ...
      <Btn onClick={save} disabled={saving}>{saving ? 'Guardando…' : 'Guardar costo de campaña'}</Btn>
    </div>
  );
}
