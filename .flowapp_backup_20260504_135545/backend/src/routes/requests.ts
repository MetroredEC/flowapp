...
router.get('/:id', async (c) => {
  ...
  const [steps, attachments, campaignCost] = await Promise.all([
    ...
    c.env.DB.prepare('SELECT id, filename, r2_key, content_type, size_bytes, is_selected, created_at FROM attachments WHERE request_id = ?').bind(id).all(),
...
router.post('/:id/attachments', async (c) => {
  ...
  if (file.size > 20 * 1024 * 1024) return c.json({ error: 'Archivo demasiado grande (máx 20 MB)' }, 400);
...
router.patch('/:id/submit', async (c) => {
  ...
  console.error('SUBMIT: calling notifyApprover for', id);
  try {
