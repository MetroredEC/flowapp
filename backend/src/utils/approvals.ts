...
export async function notifyApprover(requestId: string, level: number, env: Env): Promise<void> {
  ...
  const { subject, html, text } = buildApprovalEmail({
    ...
    requestId,
    attachments,
    approveUrl: joinUrl(apiUrl, '/approve?token=' + encodeURIComponent(approveToken)),
    rejectUrl: joinUrl(apiUrl, '/reject?token=' + encodeURIComponent(rejectToken)),
    platformUrl: frontend,
