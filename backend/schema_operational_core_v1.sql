-- FlowApp Fase Técnica 1: versiones, SLA y línea de tiempo transversal.
-- D1 ejecuta el archivo de forma atómica; no usar BEGIN/COMMIT explícitos.

ALTER TABLE process_configs ADD COLUMN default_sla_days INTEGER NOT NULL DEFAULT 5;

ALTER TABLE requests ADD COLUMN process_version_id TEXT;
ALTER TABLE requests ADD COLUMN process_version INTEGER;
ALTER TABLE requests ADD COLUMN submitted_at TEXT;
ALTER TABLE requests ADD COLUMN approved_at TEXT;
ALTER TABLE requests ADD COLUMN rejected_at TEXT;
ALTER TABLE requests ADD COLUMN cancelled_at TEXT;
ALTER TABLE requests ADD COLUMN closed_at TEXT;
ALTER TABLE requests ADD COLUMN sla_due_at TEXT;

ALTER TABLE ws_tasks ADD COLUMN started_at TEXT;
ALTER TABLE ws_tasks ADD COLUMN completed_at TEXT;
...
-- Versión inicial para los procesos que existían antes de esta migración.
...
WHERE EXISTS (
  ...
  WHERE s.space_id = ws_tasks.space_id AND s.key = ws_tasks.status AND s.is_done = 1
);

-- Línea de tiempo mínima para solicitudes históricas.
INSERT INTO work_events (id, request_id, event_type, title, actor_id, actor_name, actor_email, created_at)
...
FROM requests r
WHERE NOT EXISTS (
  SELECT 1 FROM work_events e WHERE e.request_id = r.id AND e.event_type = 'request_created'
);

INSERT INTO work_events (id, request_id, event_type, title, actor_id, actor_name, actor_email, detail_json, created_at)
SELECT lower(hex(randomblob(16))), s.request_id,
       CASE WHEN s.status = 'approved' THEN 'approval_approved' ELSE 'approval_rejected' END,
       CASE WHEN s.status = 'approved' THEN 'Aprobación registrada' ELSE 'Solicitud rechazada' END,
...
INSERT INTO work_events (id, request_id, task_id, event_type, title, actor_name, actor_email, detail_json, created_at)
SELECT lower(hex(randomblob(16))), t.source_id, t.id, 'task_created', 'Trabajo creado en el área',
...
WHERE t.source_type = 'request' AND t.source_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM work_events e WHERE e.task_id = t.id AND e.event_type = 'task_created'
  );
