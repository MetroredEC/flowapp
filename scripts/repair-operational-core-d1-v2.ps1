...
$CandidateRoots = @(
  ...
  (Resolve-Path (Join-Path $ScriptDir "../flowapp") -ErrorAction SilentlyContinue),
  (Get-Location)
) | Where-Object { $_ }

$Backend = $null
foreach ($root in $CandidateRoots) {
  $candidate = Join-Path $root "backend"
...
if (-not $Backend) {
  $Backend = "C:\Users\dbermeo\Desktop\FlowApp\flowapp\backend"
}

if (-not (Test-Path (Join-Path $Backend "package.json"))) {
  throw "No se encontro la carpeta backend. Ejecuta este script desde FlowApp o ajusta `$Backend."
}

function Invoke-D1 {
  param(
    [Parameter(Mandatory = $true)][string]$Sql,
    [string]$Title = "Ejecutar SQL en D1"
  )

  Write-Host "`n==> $Title" -ForegroundColor Cyan
  Push-Location $Backend
  try {
    # Wrangler escribe advertencias informativas en STDERR. En Windows PowerShell 5.1,
    # con ErrorActionPreference=Stop esas advertencias se convierten en NativeCommandError
    # aunque Wrangler termine correctamente. Las capturamos y validamos solo el exit code.
    $previousErrorActionPreference = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
      $outputLines = & npx wrangler d1 execute $DatabaseName `
        --env $Environment `
        --remote `
        --command $Sql `
        --yes 2>&1
...
function Ensure-D1Column {
  ...
  $result = Invoke-D1 -Sql $checkSql -Title "Validar $Table.$Column"

  if ($result -match [regex]::Escape($token)) {
    Invoke-D1 -Sql "ALTER TABLE $Table ADD COLUMN $Column $Definition;" -Title "Agregar $Table.$Column" | Out-Null
  }
  else {
...
# Columnas agregadas por schema_operational_core_v1.sql
...
Invoke-D1 -Sql $RepairSql -Title "Crear tablas y ejecutar backfill operacional" | Out-Null
...
Invoke-D1 -Sql $VerifySql -Title "Verificar reparacion"

Write-Host "`nReparacion D1 completada. No es necesario redesplegar backend ni frontend." -ForegroundColor Green
